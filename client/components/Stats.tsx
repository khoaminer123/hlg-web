      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 lg:grid-cols-4">
          {stats.map((stat, idx) => (
            <div 
              key={idx} 
              className={`flex flex-col items-center text-center px-4 py-8 group transition-all duration-300 ${
                idx !== stats.length - 1 ? 'lg:border-r border-gray-100' : ''
              }`}
            >
              <div className="mb-6 text-gray-300 group-hover:text-pink-primary transition-colors duration-500">
                <i className={`fas ${stat.icon} text-5xl font-light opacity-60 group-hover:opacity-100`}></i>
              </div>
              <div className="space-y-2">
                <div className="text-3xl md:text-4xl font-black text-pink-primary tracking-tight">
                  {stat.value}
                </div>
